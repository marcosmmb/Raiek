...
class NodeParser:
    ...
    def createWalletSet(self):
        #Creates a wallet set with 5 important tokens, must be saved
        seed = self.node.generateSeed()
        wallet_number = self.node.walletCreate()["wallet"]
        dk = self.node.deterministicKey(seed, 0)
        private = dk["private"]
        public = dk["public"]
        account = dk["account"]
        self.node.walletAdd(wallet_number, private)
    ...
    def createNewAccount(self, seed, wallet, index):
        ...
        try: 
            ...
            public = dk["public"]
            account = dk["account"]
            self.node.walletAdd(wallet, private)
        except:
            return dk
    ...
    def receiveAllXrb(self, wallet):
        ...
        self.node.searchPending(wallet)
        pending = self.node.walletPending(wallet)
        ...
        for acc in blocks:
            log[acc] = []
            for b in blocks[acc]:
                self.node.receive(wallet,acc, b)
    ...
    def sendXrb(self, wallet, source, destination, amount):
        ...
        block = self.node.send(wallet, source, destination, amount)
        try: 
            block = block["block"]
    ...
    def getRepresentative(self, wallet):
        #Returns the representative account of a wallet
        return self.node.walletRepresentative(wallet)
    #end getRepresentative

    def setRepresentative(self, wallet, representative):
        #Sets the presentative account of a wallet
        s = self.node.walletRepresentativeSet(wallet, representative)
        try:
            s = s["set"]
    ...
    def checkBlockHistory(self, block):
        ...
        return self.node.history(block)
